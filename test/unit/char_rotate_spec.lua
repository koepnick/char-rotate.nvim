...
describe('Rotation lookup', function()
  ...
  it('inserts a case-toggle rooted at the lowercase letter', function()
    char_rotate.setup({ rotations = { 'aàá', 'AÀÁ' }, append_rotations = false })
    local lower_root = char_rotate._get_next_char('lower')
    -- a -> A -> à -> á -> a
    assert.are.equal('A', lower_root['a'])
    assert.are.equal('à', lower_root['A'])
    assert.are.equal('á', lower_root['à'])
    assert.are.equal('a', lower_root['á'])
  end)

  it('inserts a case-toggle rooted at the uppercase letter', function()
    char_rotate.setup({ rotations = { 'aàá', 'AÀÁ' }, append_rotations = false })
    local upper_root = char_rotate._get_next_char('upper')
    -- A -> a -> À -> Á -> A
    assert.are.equal('a', upper_root['A'])
    assert.are.equal('À', upper_root['a'])
    assert.are.equal('Á', upper_root['À'])
    assert.are.equal('A', upper_root['Á'])
  end)
